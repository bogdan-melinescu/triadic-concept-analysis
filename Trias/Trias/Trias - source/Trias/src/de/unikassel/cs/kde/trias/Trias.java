...
/**
 * @author rja
 * @version: $Id: Trias.java,v 1.4 2009-04-23 09:13:16 rja Exp $
 * $Author: rja $
 * 
 */
public class Trias {
	 ...
	 * comparators
	 */
	private TriasComparator uComparator  = new OneDimensionComparator(Dimension.U);   // compares U
	private TriasComparator tComparator  = new OneDimensionComparator(Dimension.T);   // compares T
	private TriasComparator rComparator  = new OneDimensionComparator(Dimension.R);   // compares R
	private TriasComparator trComparator = new TwoDimensionComparator(Dimension.T,Dimension.R); // compares TxR
	...
	private void trias() throws IOException {
		...
		log.info("-------------- NEXT CLOSURE -------------- " + 
				minSupportPerDimension[Dimension.U.intValue()] + " " + 
				minSupportPerDimension[Dimension.T.intValue()] + " " + 
				minSupportPerDimension[Dimension.R.intValue()]);
		 ...
		 */
		int[] extent      = new int[numberOfItemsPerDimension[Dimension.U.intValue()] + 1]; // A
		int[] outerIntent = new int[numberOfItemsPerDimension[Dimension.U.intValue()] + 1]; // I

		int[] aPlusI; // A + i (stored values are positions in utrListe!)


		extent[0] = 0;          // we start with the empty set
		int i = uOrder[uOffsets[numberOfItemsPerDimension[Dimension.U.intValue()]]]; // we start with the largest user (pick one triple position of largest user) 

		/*
		 *  Compute the hull of the empty set.
		 */
		log.info("computing the hull of the empty set");
		...
		//System.out.println("A'' = " + getUSetFromArray(utrListe, aSet) + "   A' = " + getTrSetFromArray(utrListe, aHuelle));
		/*
		 * TODO: for this, inner next closure has to compute the concepts and output them!
		 * currently, this is not done, hence those concepts are missing (?) 
		 */
		 ...
		 */
		log.info("starting outer next closure loop");
		log.info("utrListe.length = " + itemList.length);
		log.info("uRemember.length = " + uRemember.length);
		/*
		...
		while (extent[0] < uOffsets[0] && lastI != i) {
			...
			if (outerIntent[0] >= minSupportPerDimension[T] * minSupportPerDimension[R]) {
				...
				if (aLtI (itemList, U, extent, aPlusI, i)) {
					...
					// stop, if outerIntent is empty (TODO: is this correct?)
					if (outerIntent[0] == 0) continue;
	...
	private void innerNextClosure(int[] extent, int[] relationI) throws IOException {
		 ...
		 * preprocessing
		 */
		// map extent to something similiar like utrListe TODO: minimize size of arrays!
		...
		int[] aPlusI;
		int[] modus;
		int j = tOrder[tOffsets[tCtr]]; // get largest tag
		...
		if (modus[0] >= minSupportPerDimension[R]) { // FIXME, always true, because prime returns all elements on empty set!
			...
			if (log.isDebugEnabled()) log.debug("   possible empty set hull: " + toString(extent, intent, modus, trListe));
		...
		while (intent[0] < tOffsets[0] && lastJ != j) { // stop, if intent contains all elements from T (e.g. B == T)
			...
			if (modus[0] >= minSupportPerDimension[R]) {
				...
				if (aLtI (trListe, T, intent, aPlusI, j)) {
					...
					j = tOrder[tOffsets[tCtr]]; // re-start with largest element
				} else {
	...
	private void sortByUandTxR() {
		...
		log.info("sorted by U and by TxR");
	}
	...
	private void initializePermutations() {
		...
		log.info("initialized permutations");
	}
	...
	private void writeTriples(final int[][] trListe, final int[] extent, final int[] intent, final int[] modus) throws IOException {
		int[] mappedExtent = new int[extent[0]]; for (int k=1; k<=extent[0]; k++) mappedExtent[k-1] = itemList[extent[k]][U];
		int[] mappedIntent = new int[intent[0]]; for (int k=1; k<=intent[0]; k++) mappedIntent[k-1] = itemList[trListe[intent[k]][0]][T]; 
		int[] mappedModus  = new int[modus[0]];  for (int k=1; k<=modus[0];  k++) mappedModus[k-1]  = itemList[trListe[modus[k]][0]][R]; 
	...
	private String toString (int[] extent, int[] intent, int[] modus, int[][] trListe) {
		...
		for (int k=1; k<=intent[0]; k++)  {
			buf.append(itemList[trListe[intent[k]][0]][T]);
			if (k < intent[0]) buf.append(", ");
		}
		buf.append("}, {");

		for (int k=1; k<=modus[0]; k++)  {
			buf.append(itemList[trListe[modus[k]][0]][R]);
	 ...
	 */
	private int[] prime (int[][] values, Integer[] order, int[] offsets, Integer[] xOrder, int[] xOffsets, int[] remember, int[] menge, TriasComparator comparator) {


		if (menge[0] == 0) { // empty set --> return all elements
			int[] result = new int[xOffsets.length + 1]; // hat die Größe von xOffsets
			...
			// order menge by size
			Integer[] menge_ordered = orderBySize(menge, offsets, remember);

			// copy first column to result
			int u0 = menge[menge_ordered[0]]; // smallest set
			...
			/*
			 * Schnitt der Menge result mit allen anderen Mengen (offsets[])
			 */ 
			for (int i = 1; i < menge_ordered.length; i++) {
				/*
				...
				s2intersection (result, values, order, offsets[remember[menge[menge_ordered[i]]]], offsets[remember[menge[menge_ordered[i]]] + 1] - 1, comparator);
				//} else {
